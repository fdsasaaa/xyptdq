...
p={"task":"activate_cf50_wave1_recurring_v3","status":status,"blocking_item":blocker,"runtime_root":"/var/lib/xyptdq-content/CF50-20260813-wave1","draft_count_expected":11,"scheduled_count_expected":11,"legacy_repository_scheduled_count_expected":11,"publisher_cron_count":int(cron),"source_queue":"/var/lib/xyptdq-content/CF50-20260813-wave1/scheduled","state_path":"/var/lib/xyptdq-publisher/CF50-20260813-wave1/state.json","lock_path":"/var/lib/xyptdq-publisher/CF50-20260813-wave1/publisher.lock","expected_source_intake_mode":"public_release_transfer_canary","legacy_queue_consumed":False,"cms_write_attempted":False,"wave_b_authorized":False}
...
fail_closed(){ local item="$1"; rollback; local cc; cc="$(cron_count)"; write_result BLOCKED "$item" "$cc"; echo "ACTIVATE_CF50_WAVE1_RECURRING_V3=BLOCKED item=$item" >&2; exit 10; }
...
echo "ACTIVATE_CF50_WAVE1_RECURRING_V3=PASS cron=1 scheduled=11 legacy=11"
