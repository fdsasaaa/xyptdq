...
[ "$(id -u)" -eq 0 ] || exit 3
[ -d "$REPO/.git" ] || exit 4
cron_count(){ local n=0; [ -f "$CRON_FILE" ] && n=$((n+1)); local u; u=$( (crontab -l 2>/dev/null || true) | grep -c 'run_scheduled_publish.sh' || true ); echo $((n+u)); }
write_result(){ local status="$1" blocker="$2" cron="$3"; python3 - "$RESULT_FILE" "$status" "$blocker" "$cron" <<'PY'
import json,sys
out,status,blocker,cron=sys.argv[1:]
p={"task":"activate_cf50_wave1_recurring_v4","status":status,"blocking_item":blocker,"runtime_root":"/var/lib/xyptdq-content/CF50-20260813-wave1","draft_count_expected":11,"scheduled_count_expected":11,"legacy_repository_scheduled_count_expected":11,"publisher_cron_count":int(cron),"source_queue":"/var/lib/xyptdq-content/CF50-20260813-wave1/scheduled","state_path":"/var/lib/xyptdq-publisher/CF50-20260813-wave1/state.json","lock_path":"/var/lib/xyptdq-publisher/CF50-20260813-wave1/publisher.lock","expected_source_intake_mode":"public_release_transfer_canary","identity_field":"source_article_id","legacy_queue_consumed":False,"cms_write_attempted":False,"wave_b_authorized":False}
...
rollback(){ set +e; [ "$CRON_INSTALLED_BY_TASK" = "YES" ] && rm -f "$CRON_FILE"; set -e; }
fail_closed(){ local item="$1"; rollback; local cc; cc="$(cron_count)"; write_result BLOCKED "$item" "$cc"; echo "ACTIVATE_CF50_WAVE1_RECURRING_V4=BLOCKED item=$item" >&2; exit 10; }
...
' "$POLICY" "$SCHEDULED_DIR" "$STATE_PATH" "$LOCK_PATH"; then fail_closed publication_policy_contract_mismatch; fi
...
resolved=$(realpath "$SCHEDULED_DIR"); case "$resolved" in /var/lib/xyptdq-content/*) ;; *) fail_closed runtime_source_outside_isolated_root ;; esac
...
for aid,when in expected:
 d=json.loads((dr/f'{aid}.json').read_text(encoding='utf-8')); s=json.loads((sr/f'{aid}.json').read_text(encoding='utf-8'))
 if d.get('source_article_id')!=aid or d.get('publication_state')!='draft' or 'publish_at' in d: raise SystemExit(2)
 if s.get('source_article_id')!=aid or s.get('publication_state')!='scheduled' or s.get('publish_at')!=when: raise SystemExit(3)
...
LEGACY_AFTER=$(find "$LEGACY_QUEUE" -maxdepth 1 -type f -name '*.json' 2>/dev/null | wc -l | tr -d ' '); [ "$LEGACY_AFTER" -eq 11 ] || fail_closed legacy_repository_queue_changed_during_activation
write_result PASS NONE 1
echo "ACTIVATE_CF50_WAVE1_RECURRING_V4=PASS cron=1 scheduled=11 legacy=11"
