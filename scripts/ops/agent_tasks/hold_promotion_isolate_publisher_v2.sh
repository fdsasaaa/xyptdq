#!/bin/bash
# Hold recurring promotion cron (rename out of /etc/cron.d — bulletproof) + isolate publisher state.
# Preserves the scheduled canary #1 exactly. v2: rename approach avoids sed-pattern fragility.
set -euo pipefail
umask 077
...
p = {
  "task": "hold_promotion_isolate_publisher_v2",
  "status": status,
  "phase": phase,
...
block(){ BLOCKING_ITEM="$1"; STATUS="BLOCKED"; write_result; echo "[hold-isolate-v2] BLOCKED: $1" >&2; exit 1; }
...
mv "$PROMOTION_CRON" "${PROMOTION_CRON}.held-e2e-20260902"
[ ! -f "$PROMOTION_CRON" ] || block promotion_cron_still_present
[ -f "${PROMOTION_CRON}.held-e2e-20260902" ] || block held_backup_missing
...
STATUS="PASS"
write_result
echo "HOLD_PROMOTION_ISOLATE_PUBLISHER_V2=PASS promotion_cron_held=yes publisher_state_isolated=yes canary=$CANARY_AFTER"
