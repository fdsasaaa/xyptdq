...
payload={
  "task":"publish_cf50_canary_001_v2",
  "batch_id":"CF50-20260813",
  "article_id":"LCM-CREATOR-cf50-20260813-001",
  "revision_id":"LCM-CREATOR-cf50-20260813-001:public-r1",
  "status":status,
...
fail_phase() {
  ...
  echo "CF50_CANARY_001_V2=FAIL phase=$phase" >&2
  exit 20
}

# The Server Bridge verifies the pinned task script, but the canonical website
# clone can still be behind main. Sync it before reading the publication policy.
...
php "$REPO_DIR/scripts/seo/verify_publication_seo.php" --receipt="$RECEIPT_PATH" >"$TMP_DIR/live-seo.out" 2>"$TMP_DIR/live-seo.err" || {
  ...
  echo "CF50_CANARY_001_V2=BLOCKED_AFTER_PUBLISH" >&2
  exit 21
}
...
echo "CF50_CANARY_001_V2=PASS cms_id=$CMS_ID url=$PUBLISHED_URL"
