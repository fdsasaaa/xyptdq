#!/bin/bash
# Read-only verification that recurring Publisher released CF50-032 at its normal slot.
set -euo pipefail
umask 077
...
KEY="lcm-creator-cf50-20260813-032"
ARTICLE_ID="LCM-CREATOR-cf50-20260813-032"
NEXT_KEY="lcm-creator-cf50-20260813-037"
...
p={'task':'probe_cf50_032_normal_slot_v1','status':'FAIL','phase':phase,'detail':detail,'read_only':True,'article_id':'LCM-CREATOR-cf50-20260813-032','cms_write_attempted':False,'runtime_mutated':False,'cron_mutated':False,'queue_consumed':False,'publication_attempted':False}
...
if entry.get('status')!='published': raise SystemExit('032 is not published in Publisher state')
cms=int(entry.get('cms_id') or 0)
if cms<=0: raise SystemExit('032 cms_id invalid')
if nxt.get('status')!='scheduled': raise SystemExit('037 is not still scheduled')
if int(nxt.get('cms_id') or 0)!=0: raise SystemExit('037 already has cms_id; possible premature consumption')
counts={}
for e in articles.values():
    s=str((e or {}).get('status') or 'missing'); counts[s]=counts.get(s,0)+1
if counts.get('published')!=9 or counts.get('scheduled')!=2 or counts.get('failed',0)!=0:
    raise SystemExit('unexpected Wave1 state counts after 032')
receipt=pathlib.Path(receipts)/f'{key}.{cms}.json'
if not receipt.is_file(): raise SystemExit('032 publication receipt missing')
...
if proc.returncode!=0:
    ...
    raise SystemExit('032 authoritative live SEO verification failed: '+detail)
url=f'https://www.laocaimi.org/index.php?c=show&id={cms}'
p={
 'task':'probe_cf50_032_normal_slot_v1','status':'PASS','phase':'complete','read_only':True,
 'article_id':'LCM-CREATOR-cf50-20260813-032','article_key':key,'cms_id':cms,'published_url':url,
 ...
 'next_article_id':'LCM-CREATOR-cf50-20260813-037','next_runtime_status':nxt.get('status'),
 'next_cms_id':nxt.get('cms_id'),'expected_next_publish_at':'2026-08-19T19:00:00+08:00',
 'overall_first12_seed_progress':'10/12',
 'cms_write_attempted':False,'runtime_mutated':False,'cron_mutated':False,'queue_consumed':False,'publication_attempted':False
}
with open(out,'w',encoding='utf-8') as f: json.dump(p,f,ensure_ascii=False,indent=2,sort_keys=True); f.write('\n')
PY

echo CF50_032_NORMAL_SLOT_PROBE=PASS
