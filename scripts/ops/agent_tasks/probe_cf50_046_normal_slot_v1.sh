#!/bin/bash
# Read-only verification that recurring Publisher released CF50-046 at its normal slot.
set -euo pipefail
umask 077
...
KEY="lcm-creator-cf50-20260813-046"
ARTICLE_ID="LCM-CREATOR-cf50-20260813-046"
NEXT_KEY="lcm-creator-cf50-20260813-002"
...
p={'task':'probe_cf50_046_normal_slot_v1','status':'FAIL','phase':phase,'detail':detail,'read_only':True,'article_id':'LCM-CREATOR-cf50-20260813-046','cms_write_attempted':False,'runtime_mutated':False,'cron_mutated':False,'queue_consumed':False,'publication_attempted':False}
...
if entry.get('status')!='published': raise SystemExit('046 is not published in Publisher state')
cms=int(entry.get('cms_id') or 0)
if cms<=0: raise SystemExit('046 cms_id invalid')
if nxt.get('status')!='scheduled': raise SystemExit('002 is not still scheduled')
if int(nxt.get('cms_id') or 0)!=0: raise SystemExit('002 already has cms_id; possible premature consumption')
counts={}
for e in articles.values():
    s=str((e or {}).get('status') or 'missing'); counts[s]=counts.get(s,0)+1
if counts.get('published')!=5 or counts.get('scheduled')!=6 or counts.get('failed',0)!=0:
    raise SystemExit('unexpected Wave1 state counts after 046')
receipt=pathlib.Path(receipts)/f'{key}.{cms}.json'
if not receipt.is_file(): raise SystemExit('046 publication receipt missing')
...
if proc.returncode!=0:
    ...
    raise SystemExit('046 authoritative live SEO verification failed: '+detail)
url=f'https://www.laocaimi.org/index.php?c=show&id={cms}'
p={
 'task':'probe_cf50_046_normal_slot_v1','status':'PASS','phase':'complete','read_only':True,
 'article_id':'LCM-CREATOR-cf50-20260813-046','article_key':key,'cms_id':cms,'published_url':url,
 ...
 'next_article_id':'LCM-CREATOR-cf50-20260813-002','next_runtime_status':nxt.get('status'),
 'next_cms_id':nxt.get('cms_id'),'expected_next_publish_at':'2026-08-17T19:00:00+08:00',
 'overall_first12_seed_progress':'6/12',
 'cms_write_attempted':False,'runtime_mutated':False,'cron_mutated':False,'queue_consumed':False,'publication_attempted':False
}
with open(out,'w',encoding='utf-8') as f: json.dump(p,f,ensure_ascii=False,indent=2,sort_keys=True); f.write('\n')
PY

echo CF50_046_NORMAL_SLOT_PROBE=PASS
