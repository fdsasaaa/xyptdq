...
p = {
  "task": "probe_weekly_promotion_runtime_v2",
  "status": status,
  "phase": phase,
...
PHASE="env_probe"
ENV=$(python3 - "$RUNTIME_QUEUE_ROOT" "$PUBLISHER_CRON" "$PUBLISHER_STATE_DIR" "$LEGACY_QUEUE" "$REPO" <<'PY'
import json, os, pathlib, subprocess, sys
runtime_root, cron_path, state_dir, legacy, repo = [pathlib.Path(x) for x in sys.argv[1:]]
out = {}
try:
    r = subprocess.run(['git','-C',str(repo),'status','--porcelain'], capture_output=True, text=True, timeout=10)
...
try:
    r = subprocess.run(['git','-C',str(repo),'rev-parse','--short','HEAD'], capture_output=True, text=True, timeout=10)
    out['canonical_repo_head'] = r.stdout.strip()
except Exception:
    out['canonical_repo_head'] = 'unknown' 
# publisher cron
