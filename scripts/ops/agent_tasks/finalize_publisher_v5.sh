#!/bin/bash
# Server Bridge wrapper for the already-reviewed publisher V5 finalizer.
# Raw finalizer output remains local in the agent log; only whitelisted summary
# fields are written to XYPTDQ_AGENT_RESULT_FILE.
set -euo pipefail
umask 077
...
FINALIZER="$REPO/scripts/ops/chatgpt_finalize_publisher_v5.sh"
[ -f "$FINALIZER" ] || { write_blocked 22; exit 22; }
bash -n "$FINALIZER" || { write_blocked 23; exit 23; }

TMP_OUT=$(mktemp /tmp/xyptdq-finalizer-agent.XXXXXX.log)
...
if [ "$RC" -ne 0 ]; then
  write_blocked "$RC"
  echo "publisher finalizer blocked rc=$RC" >&2
  exit "$RC"
fi

# Accept only the fixed, known summary keys emitted by the finalizer. Do not
# serialize arbitrary stdout into GitHub.
python3 - "$TMP_OUT" "$RESULT_FILE" <<'PY'
import json, re, sys
src, out = sys.argv[1:]
allowed = {
    'Recovery branch': 'recovery_branch',
    'Recovery branch remote': 'recovery_branch_remote',
    ...
    'Evidence commit remote': 'evidence_commit_remote',
...
with open(src, 'r', encoding='utf-8', errors='replace') as fh:
    for raw in fh:
        ...
        if key in allowed:
            # Summary values are intentionally constrained to printable text and
            # a small length; credentials are never accepted as a named field.
...
echo "PUBLISHER_FINALIZATION_AGENT=PASS"
