#!/bin/bash
# Read-only verification that recurring Publisher released CF50-041 at its normal slot.
set -euo pipefail
umask 077
...
KEY="lcm-creator-cf50-20260813-041"
ARTICLE_ID="LCM-CREATOR-cf50-20260813-041"
NEXT_KEY="lcm-creator-cf50-20260813-046"
...
p={'task':'probe_cf50_041_normal_slot_v1','status':'FAIL','phase':phase,'detail':detail,'read_only':True,'article_id':'LCM-CREATOR-cf50-20260813-041','cms_write_attempted':False,'runtime_mutated':False,'cron_mutated':False,'queue_consumed':False,'publication_attempted':False}
...
if entry.get('status')!='published': raise SystemExit('041 is not published in Publisher state')
cms=int(entry.get('cms_id') or 0)
if cms<=0: raise SystemExit('041 cms_id invalid')
if nxt.get('status')!='scheduled': raise SystemExit('046 is not still scheduled')
counts={}
for e in articles.values():
    s=str((e or {}).get('status') or 'missing'); counts[s]=counts.get(s,0)+1
if counts.get('published')!=4 or counts.get('scheduled')!=7 or counts.get('failed',0)!=0:
    raise SystemExit('unexpected Wave1 state counts after 041')
receipt=pathlib.Path(receipts)/f'{key}.{cms}.json'
if not receipt.is_file(): raise SystemExit('041 publication receipt missing')
proc=subprocess.run(['php',verify,f'--receipt={receipt}'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
if proc.returncode!=0: raise SystemExit('041 authoritative live SEO verification failed')
url=f'https://www.laocaimi.org/index.php?c=show&id={cms}'
p={
 'task':'probe_cf50_041_normal_slot_v1','status':'PASS','phase':'complete','read_only':True,
 'article_id':'LCM-CREATOR-cf50-20260813-041','article_key':key,'cms_id':cms,'published_url':url,
 ...
 'next_article_id':'LCM-CREATOR-cf50-20260813-046','next_runtime_status':nxt.get('status'),
 'expected_next_publish_at':'2026-08-17T10:00:00+08:00',
 'cms_write_attempted':False,'runtime_mutated':False,'cron_mutated':False,'queue_consumed':False,'publication_attempted':False
}
with open(out,'w',encoding='utf-8') as f: json.dump(p,f,ensure_ascii=False,indent=2,sort_keys=True); f.write('\n')
PY

echo CF50_041_NORMAL_SLOT_PROBE=PASS
