...
STATUS="PASS"
BLOCKING_ITEM="NONE"
VERIFY_JSON="$(mktemp /tmp/xyptdq-wave1-verify.XXXXXX.json)"
...
if [ "$STATUS" = "PASS" ]; then
  if ! python3 - "$DRAFT_DIR" "$SCHEDULED_DIR" "$VERIFY_JSON" <<'PY'
...
for article_id, publish_at in expected:
    ...
    for x,label in ((d,'draft'),(s,'scheduled')):
        ...
        if x.get('source_intake_mode') != 'sanitized_public_release_transfer':
            raise SystemExit(f"source intake mismatch {label} {article_id}")
    # Promotion must preserve source identity and content exactly.
    ...
    seen.append({'article_id':article_id,'publish_at':publish_at})
...
python3 - "$RESULT_FILE" "$STATUS" "$BLOCKING_ITEM" "$DRAFT_COUNT" "$SCHEDULED_COUNT" "$LEGACY_COUNT" "$CRON_COUNT" "$POLICY_ENABLED" "$VERIFY_JSON" <<'PY'
import json, pathlib, sys
out,status,blocker,drafts,scheduled,legacy,cron,enabled,verify=sys.argv[1:]
verified=[]
...
payload={
  'task':'verify_cf50_wave1_schedule_v1',
  'status':status,
  'read_only':True,
  'runtime_root':'/var/lib/xyptdq-content/CF50-20260813-wave1',
...
if [ "$STATUS" != "PASS" ]; then
  echo "VERIFY_CF50_WAVE1_SCHEDULE_V1=BLOCKED item=$BLOCKING_ITEM" >&2
  exit 1
fi

echo "VERIFY_CF50_WAVE1_SCHEDULE_V1=PASS drafts=$DRAFT_COUNT scheduled=$SCHEDULED_COUNT legacy=$LEGACY_COUNT cron=$CRON_COUNT"
