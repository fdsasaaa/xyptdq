...
import json,subprocess,sys,pathlib,traceback
...
try:
    ...
    problems=[]
    if entry.get('status')!='published': problems.append('049 is not published in Publisher state')
    if cms<=0: problems.append('049 cms_id invalid')
    ...
    if counts.get('published')!=11 or counts.get('scheduled',0)!=0 or counts.get('failed',0)!=0:
        problems.append('unexpected Wave1 state counts after 049')
