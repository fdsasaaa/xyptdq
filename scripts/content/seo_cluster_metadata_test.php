...
try {
    ...
    if ($code === 0 || is_file($invalidDraft)) {
    ...
    if (!is_array($mappedDraftData) || ($mappedDraftData['primary_seo_cluster_id'] ?? null) !== 'ffc_research') {
    ...
    $publicRelease = $mapped;
    unset($publicRelease['creator_batch_id']);
    $publicRelease['source_batch_id'] = 'TEST-BATCH';
    $publicReleaseInput = $tmp . '/public-release.json';
    $publicReleaseDraft = $tmp . '/public-release-draft.json';
    writeJson($publicReleaseInput, $publicRelease);
    [$code, $out] = runConverter($converter, $publicReleaseInput, $publicReleaseDraft, $editorialMapPath);
    if ($code !== 0) {
        clusterTestFail('source_batch_id editorial-map conversion failed: ' . implode("\n", $out));
    }
    $publicReleaseDraftData = json_decode((string) file_get_contents($publicReleaseDraft), true);
    if (!is_array($publicReleaseDraftData) || ($publicReleaseDraftData['primary_seo_cluster_id'] ?? null) !== 'ffc_research') {
        clusterTestFail('source_batch_id did not apply editorial map');
    }
...
fwrite(STDOUT, "[seo-cluster-metadata-test] PASS package=preserved editorial_map=applied source_batch=compatible conflicts=rejected legacy=compatible publishing=untouched\n");
