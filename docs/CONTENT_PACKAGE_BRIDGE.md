...
`content/drafts/<article_key>.json`

Draft **没有 `publish_at`**，并带：

`publication_state=draft`

因此现有 `auto_publish_filequeue.php` 不会扫描它。
...
- publication_state=draft；
- schema/article_key/title/content/catid；
- 标题和正文最低质量；
- publish_at 可解析；
- 同article_key不同内容hash拒绝覆盖；
- 同内容但不同publish_at要求显式重新排期。

输出：

`content/scheduled/<article_key>.json`
...
- Approved Package正常/错误状态/hash篡改；
- ingress幂等；
- 缺site_category_key转换失败；
- `tzjq → catid 3`；
- convert只生成draft且无publish_at；
- promote才生成scheduled并保留source_content_hash。
